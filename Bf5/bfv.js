...
const orderStats = stats.sort(function(a,b) {
...
function showImg(img,attachments,target){
    if(target == 1){
        ...
        img1.innerHTML += "<img src='./img/"+img.replace('/','')+".png' ><br>";
        img1.innerHTML +="<span>ATTACHMENTS</span><br>";
        attachments.forEach(function(att){
            img1.innerHTML +="<span>·"  + att +"</span><br>";
        });
    }else
    if(target == 2){
        img2.innerHTML = "<h3>"+img+"</h3>"; 
        img2.innerHTML += "<img src='img/"+img.replace('/','')+".png' ><br>";
        img2.innerHTML +="<span>ATTACHMENTS</span><br>";
        attachments.forEach(function(att){
            img2.innerHTML +="<span>·"  + att +"</span><br>";
        });
    }
...
function showWeaponALL(index,target){
    ...
    let nombreArma = Object.entries(arma_actual)[249];
    let nombreImg = Object.entries(arma_actual)[248];
    let atts = arma_actual.Attachments;
    weapon_stats_0.innerHTML = "";

    showImg(nombreImg[1],atts,target);
...
orderStats.forEach( function(arma){
    ...
    let w_Att = arma.Attachments;
    weaponNameList.push(w_Name.toUpperCase() + " " + w_Att);
    }
);
    ...
    for(index in weaponNameList) {
        switch (counter){
            case 0:
                ...
                break;
            case 304:
                target.options[304] = new Option("-------||||ASAULT||||--------", );
                target.options[counter].disabled = true;
                break;
            case 610:
                target.options[610] = new Option("--------||||SUPPORT||||-------", );
                target.options[counter].disabled = true;
                break;
            case 1009:
                target.options[1009] = new Option("--------||||RECON||||--------", )
                target.options[counter].disabled = true;
                break;
            case 1313:
                target.options[1313] = new Option("--------||||SIDEARMS||||--------", )
                target.options[counter].disabled = true;
                break;
        }
...
function compareStats(target){
    if (stats1Array.length !=0  && stats2Array.length != 0){
        ...
        stats1Array.forEach((stat1 , index) => {
            ...
            if (stat1 > stat2){
                //console.log(" S1-->"+index +" stat1->"+ stat1+ " stat2->" + stat2);
                s1s[index].style.fontWeight = "bold";
                s1s[index].style.color = "green";
                }else
            if(stat2 > stat1){
                //console.log(" S2-->"+index +" stat1->"+ stat1+ " stat2->" + stat2);
